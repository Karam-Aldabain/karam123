...
/**
 * Placeholder avatar shown while experts are loading or when an approved
 * expert has no profile image stored yet.
 */
const FALLBACK_AVATAR = "https://images.unsplash.com/photo-1511367461989-f85a21fda167?auto=format&fit=crop&w=1600&q=85";
...
function ExpertCard({ person }) {
    ...
    const getImageUrl = () => {
        if (imageError) return FALLBACK_AVATAR;

        const baseUrl = person.profile_image_url || person.img || FALLBACK_AVATAR;

        // Add timestamp to force reload if it's from API and not a fallback
        if (baseUrl && baseUrl !== FALLBACK_AVATAR && baseUrl.includes('/media/')) {
    ...
    return (
        <div className="w-full" style={{ perspective: 1400 }}>
            ...
            {!imageLoaded && !imageError && (
                <div className="absolute inset-0 flex items-center justify-center bg-gray-200 rounded-[30px] animate-pulse">
            ...
            >
                ...
                >
                    <img
                        ...
                        key={displayImg} // Force re-render when URL changes
                    />
                        ...
                        >
                            <div className="h-full w-full overflow-hidden rounded-full">
                                <img
                                    ...
                                    onError={handleImageError}
                                    onLoad={handleImageLoad}
                                    key={`back-${displayImg}`} // Force re-render when URL changes
                                />
                            </div>
                        </div>
...
/* -----------------------------------------------------------
   EXPERT SKELETON — shown while the API request is in-flight
----------------------------------------------------------- */
function ExpertSkeleton() {
    return (
        <div className="h-[420px] w-full animate-pulse rounded-[30px] border border-white/10 bg-white/5" />
    );
}
...
export default function LandingPage() {
    ...
    // Number of skeleton cards to show while loading (matches the visible count).
    const skeletonCount = expertVisible;
    ...
    // Only render the experts section when there is something to show (loaded
    // experts or skeleton placeholders while loading).
    const showExpertsSection = expertsLoading || experts.length > 0;
    ...
    return (
      ...
      `}</style>
            ...
            <main className="overflow-x-clip pb-16">
                ...
                {/* EXPERTS — only rendered when there is data to show or while loading */}
                {showExpertsSection && (
                    <section className="relative pt-16 pb-16 sm:pb-20" id="experts" style={darkSectionVars}>
                        ...
                        Our Experts
                                ...
                                <MotionItem variant="scaleIn">
                                    <div className="relative mt-10">
                                        {/* Navigation arrows — only show when there are real experts to navigate */}
                                        {!expertsLoading && experts.length > expertVisible && (
                                            <>
                                                <button
                                                    type="button"
                                            ...
                                            {expertsLoading ? (
                                                ...
                                                <div className="flex gap-4">
                                                    {Array.from({ length: skeletonCount }).map((_, i) => (
                                                        <div key={i} className="shrink-0 px-2" style={{ flex: `0 0 ${100 / expertVisible}%` }}>
                                                            <ExpertSkeleton />
                                                        </div>
                                                    ))}
                                                </div>
                                                ...
                                                >
                                                    {experts.map((p) => (
                                                        <div key={p.id} className="shrink-0 px-2" style={{ flex: `0 0 ${100 / expertVisible}%` }}>
                                                            <ExpertCard person={p} />
                                                        </div>
                                                    ))}
                                                </motion.div>
